 ...
 * @version 1.0
 */
public class CircularDoublyLinkedList<T extends Comparable<T>> implements 
        ListADT<T> {
    ...
    */
    ...
    public void addFirst(T element) {
        ...
        if(numNodes != 1) {
            DoublyLinkedNode<T> firstNode = first;
            temp.setNext(first);
     ...
     * Namely insert the given element after the existing element in the list
        ...
        Note: the CircularDoublyLinkedList class uses the data member variable 
        * first
        and numNodes to maintain the status of a CircularDoublyLinkedList 
        * instance.
    ...
    public void addAfter(T existing, T element) throws ElementNotFoundException,
        ...
        do
        {
            if(firstNode.getElement() == existing)
            {
                temp.setNext(firstNode.getNext());
                temp.setPrev(firstNode);
                firstNode.getNext().setPrev(temp);
        ...
        while(firstNode != first);
        throw new ElementNotFoundException("ENFE");
    }
    ...
    public T remove(T element) throws EmptyCollectionException, 
        ...
        boolean startFlag = true;
        for(DoublyLinkedNode<T> firstNode = first; (firstNode != first) || 
                startFlag; firstNode = firstNode.getNext())
        {
            if(firstNode.getElement() == foundNode.getElement()) 
            {
                firstNode.getPrev().setNext(firstNode.getNext());
                firstNode.getNext().setPrev(firstNode.getPrev());
                numNodes--;
                return null;
            }
            if(firstNode == first && startFlag == false)
            {
                throw new ElementNotFoundException("ENFE");
            }
            startFlag = false;
     ...
     * @throws EmptyCollectionException 
     */
    public DoublyLinkedNode<T> find(T element) throws ElementNotFoundException, 
            EmptyCollectionException {
        if (numNodes == 0)
            throw new EmptyCollectionException("CircularDoublyLinkedList");
     ...
     */
    public DoublyLinkedNode<T> getNode(int index) throws 
     ...
     */
    @Override
    public void set(int index, T element) throws EmptyCollectionException,
            InvalidArgumentException {
        DoublyLinkedNode<T> tmp = getNode(index);
