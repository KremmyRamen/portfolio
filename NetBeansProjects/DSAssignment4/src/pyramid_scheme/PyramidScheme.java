...
public class PyramidScheme extends LinkedTree<Person> {
    ...
    private ArrayList<MultiTreeNode<Person>>
        ...
        ArrayList<MultiTreeNode<Person>> upstream =
                new ArrayList<MultiTreeNode<Person>>();
        LinkedStack<MultiTreeNode<Person>> stack =
                new LinkedStack<MultiTreeNode<Person>>();
        if (!findNodeChain(node, target, upstream)) {
            return null;
        }
        stack.push(node);
